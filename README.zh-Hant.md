...
- 🔔 **通知(選用)。** 透過 **你自己的** Firebase 實現原生 Android MessagingStyle 推播,
  或完全不用 Firebase 的保活本機通知路徑。
...
- 🔔 **Firebase / FCM 推播。** 使用 **你自己的** Firebase 專案實現背景推播(不內建
  `google-services.json`)。它只是一個輕量 *喚醒* 訊號;訊息資料透過 WebSocket / 增量抵達。
  參見 [`docs/setup/firebase/`](docs/setup/firebase/README.md)。
- 🔋 **保活服務(Android)。** 一個前景服務,用極簡通知保持連線開啟 —— **無需** 設定推播也能
  收到提醒。預設關閉;廠商電池策略仍可能限制它。
...
- **可靠的「程式被終止」推播** 在 Android 上實際需要你自己的 `google-services.json` 和/或
  保活;否則推播只能盡力而為,其餘由 socket + 增量同步補足。
...
**[MIT](LICENSE)** · 為更願意自己架設的人而做。

[開始使用 →](docs/getting-started.zh-Hant.md)
