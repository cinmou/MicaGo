...
| **v0.11.x — server reliability** | Planned (next milestone) | Sync fidelity (lookback update pass for edits/unsends/read/delivered/errors + event-state cache + `message:update`/`message:unsend`/`chat:event`); send-error fast-fail via `message.error`; `chat.db` schema/version probing + `capabilities` in status; Messages.app-running precondition (companion owns keep-awake). | (new) `internal/relaydb` update pass + state cache, `internal/store` schema probe, `internal/realtime` events, `internal/httpapi` capabilities; companion runtime UX | TBD (unit tests over fixture `chat.db`) | [`spec-v0.11.x-server-reliability.md`](spec-v0.11.x-server-reliability.md) | Implement in the order in the spec (schema probe → update pass → send-error → messages-running → group events → companion UX). |
| **v0.12 — Firebase self-host push + URL sync** | Planned (after v0.11.x) | Self-hosted Firebase: real FCM push delivery; optional Firestore sync of the **public URL only**. Strict privacy (no content/contacts/numbers/tokens/attachments/history). | (new) `internal/notify/fcm_*`, optional `internal/notify`/config for Firestore URL sync | TBD | [`CURRENT_PLAN.md`](CURRENT_PLAN.md) (spec to be written: `spec-v0.12.0-firebase-self-host.md`) | Do **after** v0.11.x reliability lands; write the v0.12 spec, then implement FCM delivery first. |
