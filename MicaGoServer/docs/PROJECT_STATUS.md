...
| **v0.11.5 — message fidelity** | In validation | Conservative, **additive** patch: attachment `attachmentKind` (image/video/audio/file/sticker/unknown), `isVoiceMessage` (CAF UTI), `uti`, `isSticker`; UTI/extension **MIME inference** (REST + WS + download `Content-Type`); typedstream **`+!`/`+$` text-extraction fix**. Safe path handling unchanged. Effect fields (`expressiveSendStyleID`/`balloonBundleID`) + message-level `is_audio_message` **deferred** (documented). | `internal/store/{text,attachmentkind,models,queries}.go`, `internal/relaydb/{migrations,sync,query}.go`, `internal/httpapi/handlers.go`; `relay.db` `attachments.uti`/`is_sticker` | `attachmentkind_test.go`, `text_test.go` (+`+!`/`+$` prefix tests); `go test ./...` green | [`spec-v0.11.5-message-fidelity.md`](spec-v0.11.5-message-fidelity.md), [`spec-v0.9.0-client-api-contract.md`](spec-v0.9.0-client-api-contract.md) | Live-verify image/HEIC/PDF/voice/video on a Mac with FDA; then resume v0.12. |
| **v0.12 — Firebase self-host push + URL sync** | In validation (offline) | Real `fcm` provider (service-account JWT→OAuth2, FCM HTTP v1 per-token send, 24h TTL, UNREGISTERED token pruning, payload-size tolerance, `previewMode` gating); optional Firestore **public-URL-only** sync; `POST /api/server/notifications` live config; status `implemented` includes `fcm` when configured; push gated by v0.11.3 rules. Strict privacy (no content/contacts/numbers/tokens-in-public-docs/attachments/history). | `internal/notify/{serviceaccount,fcm,firestore,dispatcher}.go`, `internal/config` (`firebase.*`, `UpdateNotificationsConfig`), `internal/httpapi/notifications.go`, `internal/relaydb/devices.go` (`ClearDevicePushToken`); companion `NotificationsView.swift`, Devices Test Push; setup docs `docs/setup/firebase/*` | `notify` unit tests (SA parse, JWT, payload, classify, prune); `go test ./...` green; `xcodebuild` green. Live FCM/Firestore needs a real Firebase project. | [`spec-v0.12.0-firebase-self-host.md`](spec-v0.12.0-firebase-self-host.md), [`setup/firebase/README.md`](setup/firebase/README.md) | Live-verify with a real Firebase project + Android device; then close. |
