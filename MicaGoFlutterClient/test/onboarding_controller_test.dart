...
PairingPayload _payload(ConnectionMode mode, {bool lan = true, bool public = true}) =>
    PairingPayload(
      version: 2,
      mode: mode,
      token: 'tok',
      endpoints: [
        if (lan)
          const PairingEndpoint(
              kind: EndpointKind.lan, baseUrl: 'http://192.168.1.5:3000', priority: 1),
        if (public)
          const PairingEndpoint(
              kind: EndpointKind.public, baseUrl: 'https://pub.example.com', priority: 2),
      ],
    );
...
void main() {
  test('LAN connects first; profile uses LAN endpoint', () async {
    final probed = <String>[];
    final c = _controller(prober: (e, _) async {
      probed.add(e.baseUrl);
      return true; // everything reachable
    });
    final profile = await c.run(_payload(ConnectionMode.lanFirst), ConnectionMode.lanFirst);
  ...
  test('LAN fails → falls back to Public; active endpoint = public', () async {
    final c = _controller(prober: (e, _) async => e.kind == EndpointKind.public);
    final profile = await c.run(_payload(ConnectionMode.lanFirst), ConnectionMode.lanFirst);
  ...
  test('LAN-only never tries Public and fails cleanly when LAN is down', () async {
    final probed = <String>[];
    final c = _controller(prober: (e, _) async {
      probed.add(e.baseUrl);
      return false; // LAN down
    });
    final profile = await c.run(_payload(ConnectionMode.lanOnly), ConnectionMode.lanOnly);
  ...
  test('sync failure is non-fatal; still reaches done', () async {
